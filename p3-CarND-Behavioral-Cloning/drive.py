...
def add_random_brightness(image, rand_val=.25+np.random.uniform()):
    ...
    image_tran = np.array(image_tran, dtype = np.float64)
    random_bright = .5+np.random.uniform()
    image_tran[:,:,2] = image_tran[:,:,2]*random_bright
    image_tran[:,:,2][image_tran[:,:,2]>255]  = 255
    image_tran = np.array(image_tran, dtype = np.uint8)
    image_tran = cv2.cvtColor(image_tran,cv2.COLOR_HSV2RGB)
    image_tran[:,:,2] = image_tran[:,:,2]*rand_val
    return image_tran

def pre_processImage(img):
    #image = add_random_brightness(img,.25+np.random.uniform())
...
def telemetry(sid, data):
    if data:
        ...
        image = pre_processImage(image_array)
